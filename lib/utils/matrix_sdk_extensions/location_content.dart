...
extension BeaconTimelineExtension on Timeline {
  /// The beacons which are being shared in this room at [time], the most
  /// recent one per user.
  List<Event> beaconsRunningAt(DateTime time) {
    final newestPerUser = <String, Event>{};
    for (final event in events) {
      if (!BeaconEventTypes.beaconInfo.contains(event.type)) continue;
      final user = event.stateKey ?? event.senderId;
      final previous = newestPerUser[user];
      if (previous == null ||
          event.originServerTs.isAfter(previous.originServerTs)) {
        newestPerUser[user] = event;
      }
    }
    return newestPerUser.values
        .where((event) => event.isBeaconRunningAt(time))
        .toList();
  }
}
